...
**Название пазла:** Network Cabling  
    
**Задача:**  
Интернет-оператор планирует подключить бизнес-парк к оптоволоконной сети. Зона покрытия большая, и оператор просит вас написать программу, которая рассчитает минимальную длину оптоволоконного кабеля, необходимого для подключения всех зданий.
  
Для выполнения работ у оператора имеются технические ограничения, в связи с чем он вынужден действовать следующим образом:
Основной кабель пересечет парк с запада на восток (от точки x самого западного здания до точки x самого восточного здания).  

Для каждого здания выделенный кабель будет соединяться от здания к основному кабелю по минимальному пути (север или юг).  
  
### Решение  
Так как в задании сказано что должен быть основной кабель идущий с запада на восток, то посчитать длину этого кабеля можно вычислив разницу между координатами по оси X самого западного и самого восточного зданий. Чтобы длина кабеля была минимальной, координата Y основного кабеля должна быть выбрана таким образом, чтобы количество зданий севернее кабеля и южнее кабеля было одинаково. Для того чтобы выбрать координату Y вектор с координатами зданий сортируется по координате Y, в качестве координаты основного кабеля выбирается элемент находящийся в середине отсортированного вектора.  

  
# Stock_Exchange_Losses.cpp  

**Название пазла:** Stock Exchange Losses   
    
**Задача:**  
Финансовая компания проводит исследование худших инвестиций в акции и хотела бы приобрести программу для этого. Программа должна анализировать хронологический ряд стоимости акций, чтобы определить максимальный убыток, который можно получить, купив акцию в момент времени t0 и продав её позднее, в момент времени t1. Убыток будет выражен как разница в стоимости между моментами t0 и t1. Если убытка нет, убыток будет равен 0.  
  
  
### Решение  
В цикле производится поиск максимальной разницы между началом промежутка падения стоимости и концом промежутка. Когда начинается промежуток падения стоимости, то запоминается его начальная цена, когда цена начинает расти тогда вычисляется разница между началом промежутка и началом роста.  

  
  
# The_Labyrinth.cpp  

**Название пазла:** The Labyrinth   
    
**Задача:**  
Структура представляет собой прямоугольный лабиринт, состоящий из ячеек. Внутри лабиринта Рик может двигаться в любом из следующих направлений: UP, DOWN, LEFT or RIGHT.  

Рик использует свой трикодер для сканирования области вокруг себя, но из-за поля-дизраптора он может сканировать только клетки, расположенные в квадрате шириной в 5 клеток с центром на нем.  

К сожалению, Спок был прав: здесь ловушка! Как только вы доберётесь до рубки управления, запустится обратный отсчёт, и у вас останется лишь ограниченное количество патронов до срабатывания сигнализации. Как только сигнализация сработает, Рик обречён...  

Рик умрет, если произойдет что-либо из следующего:
- У реактивного ранца Рика закончилось топливо. У вас его хватит на 1200 движений.
- Рик не успевает достичь стартовой позиции до срабатывания будильника. Обратный отсчёт времени до срабатывания будильника начинается после достижения пункта управления.
- Рик касается стены или земли: его разрывает механическая ловушка.  

Вы добьетесь успеха, если доберетесь до диспетчерской и вернетесь на исходную позицию до того, как сработает сигнализация.  
  
  
### Решение  
Для поиска путей используется поиск в ширину, так как с его помощью можно найти кратчайший путь. Пока комната управления не обнаружена, Рик движется в направлении ближайшего знака вопроса, т.е. исследует лабиринт. Когда комната управления обнаружена, то ищется путь до нее, если пути до нее нет, то продолжается исследование. Когда Рик достиг комнаты управления, то ищется кратчайший путь до стартовой точки.  

  
  
# Conway_Sequence.cpp  

**Название пазла:** Conway Sequence  
  
    
**Задача:**  
Внимание! Эта последовательность может вас заболеть. Причина проста, но необычна: прочитайте строку вслух, глядя на строку выше, и вы заметите, что каждая строка (кроме первой) представляет собой инвентаризацию предыдущей.  
Эта последовательность относится к методу кодирования диапазонов для сжатия идентичных значений без потери информации. Этот метод применяется, в частности, для сжатия изображений.  
Ваша задача — написать программу, которая будет отображать строку L этой серии на основе исходного числа R.  
  
  
### Решение  
Подход состоит в том чтобы постепенно от первой строки дойти до L строки используя алгоритм преобразования. При этом все предыдущие строки сохраняются в вектор. Когда дошли до L строки, то цикл заканчивается. В ответ выводится L элемент вектора.